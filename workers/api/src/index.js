...
async function handleCheckoutSessions(request, env) {
  ...
  try {
    accessToken = await paypalGetAccessToken(env);
      ...
      // Last-ditch handler: never depend on helper functions that might throw.
      const body = {
      ...
      try {
        Object.assign(headers, withCors(request, env));
      } catch (_) {
        // If CORS helper blows up, still return JSON.
      }
